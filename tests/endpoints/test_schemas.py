...
@pytest.mark.parametrize(
    "phq_labels,raise_validation_error",
    [
        ({"label": 34, "weight": "holiday"}, True),  # wrong type
        ...
        ({"label": "another_string", "weight": 7}, False),
    ],
)  # correct
...
def test_resultset():
    ...
    assert p1.count == 9
    assert list(p1) == [
        ResultExample(**{"value": 1}),
        ResultExample(**{"value": 2}),
        ResultExample(**{"value": 3}),
    ]
    ...
    assert list(p2) == [
        ResultExample(**{"value": 4}),
        ResultExample(**{"value": 5}),
        ResultExample(**{"value": 6}),
    ]
    ...
    p3 = p2.get_next()
    assert list(p3) == [
        ResultExample(**{"value": 7}),
        ResultExample(**{"value": 8}),
        ResultExample(**{"value": 9}),
    ]
    ...
    assert [x.model_dump() for x in p1.iter_pages()] == [
        endpoint.load_page(page=2).model_dump(),
